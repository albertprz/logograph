...
object QueryImpl:
  ...
  def selectAll[T <: DbDataSet](using
      Quotes,
      Type[T]
  ): Expr[SelectStatement[T]] = buildQueryAll[T]

  def select[T, R <: DbDataSet](
      query: Expr[T => Query[R]],
      subQueries: Expr[Seq[SelectStatement[?]]]
  )(using Quotes, Type[T], Type[R]): Expr[SelectStatement[R]] =
    buildQuery[T, R](query, subQueries)

  def updateAll[T <: DbTable](
      setMap: Expr[T => Map[Any, Any]]
  )(using Quotes, Type[T]): Expr[UpdateStatement[T]] = buildUpdate[T](setMap)

  def update[T <: DbTable](
      setMap: Expr[T => (Map[Any, Any], Where)]
  )(using Quotes, Type[T]): Expr[UpdateStatement[T]] = buildUpdate[T](setMap)

  def deleteAll[T <: DbTable](using Quotes, Type[T]): Expr[DeleteStatement[T]] =
    buildDelete[T](None)

  def delete[T <: DbTable](
      where: Expr[T => Where]
  )(using Quotes, Type[T]): Expr[DeleteStatement[T]] =
    buildDelete[T](Some(where))

  def insert[T <: DbTable](
      data: Expr[T]
  )(using Quotes, Type[T]): Expr[InsertStatement[T]] =

    val typeInfo = extractTypeInfo[T]
    '{ InsertStatement(Left(List($data)), ${ Expr(typeInfo) }) }

  def insertSeq[T <: DbTable](
      data: Expr[Seq[T]]
  )(using Quotes, Type[T]): Expr[InsertStatement[T]] =
  ...
  def insertQuery[T <: DbTable](
      query: Expr[SelectStatement[T]]
  )(using Quotes, Type[T]): Expr[InsertStatement[T]] =
...
private def buildQueryAll[T <: DbDataSet](using
    Quotes,
    Type[T]
): Expr[SelectStatement[T]] =
...
private def buildUpdate[T <: DbTable](
  ...
  val (clause, params) =
    extractor.getUpdateClause(updateTree.asTerm, typeInfo.fullClassName)
...
private def buildDelete[T <: DbTable](
    whereTree: Option[Expr[T => Where]] = None
)(using Quotes, Type[T]): Expr[DeleteStatement[T]] =
  ...
  val (clause, params) =
    extractor.getDeleteClause(whereTree.map(_.asTerm), typeInfo.fullClassName)
...
private def extractTypeInfo[T: Type](using Quotes) =
  ...
  val elemTypes =
    fields.map(_.tree match
      case v: ValDef => v.tpt.tpe.typeSymbol.name)
