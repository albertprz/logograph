...
import com.albertprz.logograph.utils as utils
...
class QueryExtractor(val quotes: Quotes):
  ...
  def getUpdateClause(updateTree: Tree, typeName: String) =
    ...
    val setMap =
      mapArgs.map { case (key, value) =>
        (getField(key).get, getSQLExpressions(value).head)
      }.toMap
  ...
  def getDeleteClause(whereTree: Option[Tree], typeName: String) =
    ...
    if whereTree.isDefined then init(whereTree.get, typeName)
  ...
  private def getSelectClause(
    ...
    if distinctAllArgs.nonEmpty then
      Some(SelectDistinctAllClause(distinctAllArgs.head))
    else if distinctArgs.nonEmpty then
  ...
  private def getJoinClauses(tree: Tree) =
    ...
    val args =
      ...
      yield (
        ...
        argList(1).flatMap(getSQLExpressions)
      )
  ...
  private def getSQLExpressions(tree: Tree): List[SQLExpression] =
    ...
    val expressions =
      for exprTree <- exprTrees
      yield List(
        getField(exprTree),
        getOperation(exprTree),
        getLiteral(exprTree),
        getIdentity(exprTree)
      ).flatten.headOption
  ...
  private def getOperation(tree: Tree) =
    ...
    val op: Option[(String, List[Term])] =
      tree match
        case Apply(TypeApply(Ident(operator), _), operands)      =>
          Some((operator, operands))
        case Apply(Apply(Ident(operator), operands1), operands2) =>
          Some((operator, operands1 ++ operands2))
  ...
  private def getLiteral(tree: Tree): Option[LiteralVal] =
    ...
    tree match
      ...
      case Apply(TypeApply(Select(Ident(id), _), _), List(block))
          if id.toString.contains("List") =>

        val list =
          getSeqLiteralElems(block.asInstanceOf[Typed].expr)
  ...
  private def getIdentity(tree: Tree) =
    ...
    val identity =
      tree match
        case ident @ Select(Select(Select(Select(_, _), _), _), _) =>
          Some(ident)
  ...
  def findMapArgs(tree: Tree) =
    ...
    val mapEntries =
      findAll(tree)(_ match
        case Apply(
              TypeApply(Select(Apply(TypeApply(Ident(arrow), _), _), _), _),
              _
            ) =>
    ...
    yield mapEntry match
      case Apply(TypeApply(Select(Apply(_, key), _), _), value) =>
        (key.head, value.head)
  ...
  def findTypedCtorArgs(tree: Tree, className: String) =
    ...
    val ctor =
      findAll(tree)(t =>
        t match
          case Apply(Apply(TypeApply(Select(Ident(x), _), _), _), _) =>
            x.toString.contains(className)
          case Apply(TypeApply(Select(Ident(x), _), _), _)           =>
            x.toString.contains(className)
          case _                                                     => false
      )
    ...
    val ctor =
      findAll(tree)(t =>
        t match
          ...
          case Apply(TypeApply(Select(Ident(x), _), _), _)           =>
            x.toString.contains(className)
          case Apply(Apply(Select(Ident(x), _), _), _)               =>
            x.toString.contains(className)
  ...
  private def extractCaseDefArgs(tree: Tree) =
    ...
    val args =
      tree match
        case CaseDef(caseDef, _, _) =>
          findAll(caseDef)(_ match
            case _: Bind => true
            case _       => false)
        case _                      => List.empty
  ...
  private def findAll(tree: Tree)(filterFn: Tree => Boolean): List[Tree] =
    ...
    val acc =
      new TreeAccumulator[List[Tree]]:
        def foldTree(matchedTrees: List[Tree], tree: Tree)(
            owner: Symbol
        ): List[Tree] =
