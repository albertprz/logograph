...
object Query {
  ...
  def apply[T <: DbDataSet](select: SelectBase[T], where: Where) = new Query[T]

  def apply[T <: DbDataSet](select: SelectBase[T], orderBy: OrderBy) =
    new Query[T]

  def apply[T <: DbDataSet](
      select: SelectBase[T],
      where: Where,
      orderBy: OrderBy
  ) = new Query[T]

  def apply[T <: DbDataSet](select: SelectBase[T], joins: BaseJoin*) =
    new Query[T]
