...
class LogographTest extends AnyFunSpec with BeforeAndAfter {
  ...
  describe("Logograph") {
    ...
    it("can retrieve query results") {
      ...
      assert(
        complexQuery.run().unsafeRunSync().head == (Result(
          SeedData.john.name,
          SeedData.john.age,
          SeedData.johnAddress.street,
          SeedData.johnTelephone.number
        ))
      )
    }
    ...
    it("can insert records into the database") {
      ...
      context
        .run(insert(matt), insert(mattAddress), insert(mattTelephone))
        .unsafeRunSync()
      ...
      assert(
        selectAll[Address]
          .run()
          .unsafeRunSync() == SeedData.addresses :+ mattAddress
      )

      assert(
        selectAll[Telephone]
          .run()
          .unsafeRunSync() == SeedData.telephones :+ mattTelephone
      )
    }
    ...
    it("can update records from the database") {
      ...
      context
        .run(
          update[Person](p =>
            (Map(p.age -> 40, p.isEmployer -> true), Where(p.name === "John"))
          )
        )
        .unsafeRunSync()

      assert(
        selectAll[Person].run().unsafeRunSync() == Seq(
          SeedData.john.copy(age = 40, isEmployer = true),
          SeedData.thomas
        )
      )
    }
    ...
    it("can delete records from the database") {
      ...
      context
        .run(
          delete[Person](p => Where(p.age < 33)),
      ...
      assert(
        selectAll[Address].run().unsafeRunSync().head == SeedData.thomasAddress
      )
