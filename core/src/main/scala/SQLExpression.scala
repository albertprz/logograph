...
case class Operation(operator: String, operands: List[SQLExpression])(using
    LogographConfig
) extends SQLExpression:
  ...
  val opType: OpType =
    if infixOps.contains(operator) then Infix
    else if postfixOps.contains(operator) then Postfix
    else Prefix

  override lazy val validate =

    if !allOps.contains(operator) then Some(InvalidOperator(operator, allOps))
    else None
...
case class Table(tableName: String)(using cfg: LogographConfig)
  ...
  lazy val sql =
    tableName
      .convert(cfg.tableConverter)
      .convertCase(cfg.tableCaseConverter)
      .wrapBrackets()

case class Column(columnName: String, tableName: String)(using
    cfg: LogographConfig
) extends SQLClause:
      ...
      case op: Operation =>
        op.operands
          .flatMap(
            findAggFields(
              _,
              isAgg || SQLOperations.aggOps.contains(op.operator)
            )
          )
      case _             => List.empty
...
private object Predicate:
  ...
  def adaptSql(exp: SQLExpression)(using LogographConfig) =
    exp match
