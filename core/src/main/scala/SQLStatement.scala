...
import com.albertprz.logograph.core.{Operator, SQLExpressionClause}
...
private object SQLStatement:
  ...
  def getParamList(params: Map[String, Any]) =
    params.values.flatMap {
      _ match
        case list: List[?] => list
        case other: Any    => List(other)
    }.toList

  def getSQL(sqlTemplate: String, params: Map[String, Any])(using
      LogographConfig
  ) =
    params.values
      .filter(_.isInstanceOf[List[Any]])
      .foldLeft(sqlTemplate) { case (acc, curr: List[?]) =>
        acc.replaceFirst(
          s"${Operator("in").sql} [?]",
          curr
            .map(x => "?")
