...
import config.LogographConfig
import utils.TypeInfo
import utils.StringUtils.*
import utils.TreeUtils.*

case class SelectStatement[T <: DbDataSet](
    sqlTemplate: String,
  ...
  def ++(select: SelectStatement[T]) = this.union(select)

  def --(select: SelectStatement[T]) = this.except(select)

  def &&(select: SelectStatement[T]) = this.intersect(select)
...
case object SelectStatement:
  ...
  private def concat[T <: DbDataSet](
      selects: Seq[SelectStatement[T]],
      separator: String
  ) =
  ...
  private def generate[T <: DbDataSet](select: SelectStatement[?]) =
    ...
    val queries =
      buildTree(select.asInstanceOf[SelectStatement[DbDataSet]])(_.subQueries)
        ...
        .map { select =>
          (
            select.tableNames
              .zip(select.dependencies)
              .foldLeft(select.sqlTemplate) { case (querySql, (tableName, i)) =>
                querySql.replaceFirst(s"\\[${tableName.unwrap()}\\]", s"[q$i]")
              },
            select.index
          )
        }
    ...
    val sqlTemplate =
      if queries.init.nonEmpty then s"$ctes\n$finalQuery"
      else finalQuery

    val params =
      (select.subQueries.map(_.params) :+ select.params).flatten.toMap
