...
import utils.{CaseConverter, Error, FileUtils, StringCase}
...
case class LogographConfig(
    engine: Option[SQLEngine] = None,
...
object Implicits:
  ...
  given ConfigReader[CaseConverter] =
    ConfigReader.fromNonEmptyStringTry(
      StringCase(_).map(CaseConverter.apply).toTry
    )
