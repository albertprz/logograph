...
object StringUtils:
  ...
  extension (str: String)
    ...
    def wrap(startStr: String, endStr: String) = s"$startStr$str$endStr"

    def unwrap() = str.substring(1, str.length - 1)

    def wrapBrackets() = str.wrap("[", "]")

    def wrapParens() = str.wrap("(", ")")

    def wrapCurlyBrackets() = str.wrap("{", "}")
