  ...
  def map[U](mapFn: T => U): Tree[U] =
    this match
      ...
      case Tree.Node(value, children) =>
        Node(mapFn(value), children.map(_.map(mapFn)))

  def foldLeft[U](init: U)(reduceFn: (U, T) => U): U =
    this match
      case Tree.Leaf(value)           => reduceFn(init, value)
      case Tree.Node(value, children) =>
        (children :+ Leaf(value)).foldLeft(init) { case (acc, curr) =>
          curr.foldLeft(acc)(reduceFn)
        }
...
object TreeUtils:
  ...
  def buildTree[T](treeObject: T)(treeFn: T => Seq[T]): Tree[T] =
    if treeFn(treeObject).isEmpty then Leaf(treeObject)
    else Node(treeObject, treeFn(treeObject).map(buildTree(_)(treeFn)))
