...
import com.albertprz.logograph.core.{SQLExpression, Field}
...
object Error:
  ...
  case class CaseNotAllowed (elem: String, elemType: String, set: Set[?])
      extends ConfigError(s"Element $elem does not belong to the set of allowed $elemType: $set")
  ...
  case class InvalidOperator (operator: String, validOperators: Set[String])
      extends SQLError(s"Operator $operator does not belong to the set of allowed operators: $validOperators")
  ...
  case class InvalidParameterType (value: Any)
      extends SQLError("Unknown types cannot be used in queries " +
                       "for constant or runtime parameter values: \n" +
                      s"""|Type: ${value.getClass.getName()}
                          |Value: $value""".stripMargin)

  case class InvalidQueryResultType (value: Any)
      extends SQLError("Unknown types cannot be returned " +
                       "as query results: \n" +
  ...
  case class InvalidExpression ()
      extends OrderByError("Only fields and operations are allowed in an Order By Clause")
